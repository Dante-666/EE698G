...
static int
print_msg (Parser * p, const char * format, ...)
{
    va_list args;
...
{
    const char * path = getenv ("DGC_CONFIG_PATH");
...
void config_get_random_numbers (int min, int max, int *index, int num) { 
    srand (time (NULL));
    for (int s = 0; s < num; s++) {
        int rand_index = rand()%(max - min) + min;
        if (s == 0) {
            index[s] = 1; //rand_index;
            continue;
        }
        int unique = 1;
        while (1) {
            for (int temp = 0; temp < s; temp++){
                if ((rand_index - index[temp] == 0)) unique = 0;
            }
            if (unique) break;
            unique = 1;
